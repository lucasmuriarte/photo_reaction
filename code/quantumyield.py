...
class QuantumYields():    
    ...
    def photoReactionDiff(self, C, time, Fi_A, Fi_B, Eps, I, K_A):
        ...
        """
        return odeint(self.photoReaction, C, time,
                      args=(Fi_A, Fi_B, Eps, I, K_A))
    ...
    def optimizationGlobal(self, params, time_all, data_all):
        ...
        for i in range(len(data_all)):
            ...
            fit = self.photoReactionDiff(C, time, params['Fi_A'].value,
                                         params['Fi_B'].value, Eps, I, thermal)
        ...
        if n > 1:
            ...
            for i, plot in enumerate(plot):
                    ...
                    fit = self.photoReactionDiff(C,
                                                 ...
                                                 I,
                                                 thermal)
                    datos = plot.plot(time, data, color=color,
                                      label=str('raw data'))
                    plt.setp(datos[1:], label="_")
                    fites = plot.plot(time[self.index:],
                                      fit, 'r', label=str('fit'))
                    plt.setp(fites[1:], label="_")
                    plot.set_xlabel('Time (s)', size=14)
                    plot.set_title(experiment[i], size=14)
                    plot.ticklabel_format(style='sci',
                                          scilimits=(-3, 4),
                                          axis='y')
                    plot.legend(loc=7, prop={'size': 14})
                    plot.tick_params(axis='both', which='major', labelsize=14)
            ...
            datos = plot.plot(time, data,
                              color=color,
                              label=str('raw data'))
            plt.setp(datos[1:], label="_")

            fites = plot.plot(time[self.index:],fit,
                              'r', label=str('fit'))
            plt.setp(fites[1:], label="_")
